...
for (i in 1:length(lab)){
  load(paste0("output_out3mes_nolag/", lab[i],".RData"))
...
fwrite(table1, file = "results_out3mes_nolag/table_fitting.csv", quote = FALSE, 
       row.names = FALSE) 
...
selected.model <- "formula.2.4a"
selected.model <- "formula.1.2a"   #modificar el mapa de cantones por año
selected.model <- "formula.3.4a"

load(paste0("output_out3mes_nolag/",selected.model,".RData"))
...
ggsave(month.effect.canton, 
       filename = paste0("results_out3mes_nolag/",selected.model,".month.effect.canton.jpg"), 
       height = 20, width = 20)
...
ggsave(canton.effect, 
       filename = paste0("results_out3mes_nolag/",selected.model,".canton.effect.jpg"), 
...
ggsave(prediction.in, 
       filename = paste0("results_out3mes_nolag/",selected.model,".prediction.Case.in.jpg"), 
       height = 20, width = 20)
...
ggsave(prediction.out, 
       filename = paste0("results_out3mes_nolag/",selected.model,".prediction.Case.out.jpg"), 
       height = 20, width = 20)
...
ggsave(prediction.in, 
       filename = paste0("results_out3mes_nolag/",selected.model,".prediction.RR.in.jpg"), 
       height = 20, width = 20)
...
ggsave(prediction.out, 
       filename = paste0("results_out3mes_nolag/",selected.model,".prediction.RR.out.jpg"), 
       height = 20, width = 20)
...
write_csv(metricas_tot_in,file = paste0("results_out3mes_nolag/",selected.model,".metricas.in.csv"))
write_csv(metricas_tot_out,file = paste0("results_out3mes_nolag/",selected.model,".metricas.out.csv"))
