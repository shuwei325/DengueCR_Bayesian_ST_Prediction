...
df <- data %>% 
  dplyr::select(Canton,nCanton,time,Year,Month,RR,Cases,Poblacion,OFF,clusterRR,clusterPrecip,clusterMVC4,clusterMVC5,clusterMVC6) %>%
  mutate(Y = Cases) 
...
baseformula3 <- Y ~ 1 + f(Month, replicate = clusterMVC4, model = "rw1", cyclic = TRUE, constr = TRUE,
                          scale.model = TRUE,  hyper = precision.prior) + f(nCanton, model="iid") + 
  basis_RR + basis_Precip + basis_Nino3SSTA + basis_TNA

baseformula3.1 <- Y ~ 1 + f(Month, replicate = clusterMVC5, model = "rw1", cyclic = TRUE, constr = TRUE,
                            scale.model = TRUE,  hyper = precision.prior) + f(nCanton, model="iid") + 
  basis_RR + basis_Precip + basis_Nino3SSTA + basis_TNA

baseformula3.2 <- Y ~ 1 + f(Month, replicate = clusterMVC6, model = "rw1", cyclic = TRUE, constr = TRUE,
                            scale.model = TRUE,  hyper = precision.prior) + f(nCanton, model="iid") + 
  basis_RR + basis_Precip + basis_Nino3SSTA + basis_TNA
...
baseformula5 <- Y ~ 1 + f(Month, replicate = nCanton, model = "rw1", cyclic = TRUE, constr = TRUE,
                          scale.model = TRUE,  hyper = precision.prior) + f(nCanton, model="iid") + 
  basis_RR + basis_Precip + basis_Nino3SSTA + basis_TNA + 
  #basis_Nino12SSTA + basis_Nino4SSTA + basis_Nino34SSTA +
  basis_EVI + basis_NDVI + basis_NDWI + basis_LSD + basis_LSN

baseformula6.1 <- Y ~ 1 + f(Month, replicate = clusterMVC4, model = "rw1", cyclic = TRUE, constr = TRUE,
                            scale.model = TRUE,  hyper = precision.prior) + f(nCanton, model="iid") + 
  basis_RR + basis_Precip + 
  basis_TNA + 
  basis_Nino34SSTA +  basis_NDVI + 
  basis_LSD 

baseformula6.2 <- Y ~ 1 + f(Month, replicate = nCanton, model = "rw1", cyclic = TRUE, constr = TRUE,
                            scale.model = TRUE,  hyper = precision.prior) + f(nCanton, model="iid") + 
  basis_RR + basis_Precip + 
  basis_TNA + 
  basis_Nino34SSTA +  basis_NDVI + 
...
#model 3: Month |clusterMVC4
#model 3.1: Month |clusterMVC5
#model 3.2: Month |clusterMVC6
#model 4: Month |Canton

#model 5: model 4 + all variables

#model 6.1: model +selected variables + Month |clusterMVC4
#model 6.2: model +selected variables + Month |Canton
...
ggsave(month.effect.canton, filename = "figs/Model5.month.effect.canton.jpg", height = 20, width = 20)
...
ggsave(canton.effect, filename = "figs/Model5.canton.effect.jpg", height = 20, width = 20)
...
ggsave(prediction.in, filename = "figs/Model5.prediction.Case.in.jpg", height = 20, width = 20)
...
ggsave(prediction.out, filename = "figs/Model5.prediction.Case.out.jpg", height = 20, width = 20)
...
ggsave(prediction.in, filename = "figs/Model5.prediction.RR.in.jpg", height = 20, width = 20)
...
ggsave(prediction.out, filename = "figs/Model5.prediction.RR.out.jpg", height = 20, width = 20)
...
metricas_tot_in <- base_grafico_in_g %>% group_modify(~metricas(.x))
metricas_tot_out <- base_grafico_out_g %>% group_modify(~metricas(.x))
write_csv(metricas_tot_in,file = 'figs/metricas5_Canton_INLA_in.csv')
write_csv(metricas_tot_out,file = 'figs/metricas5_Canton_INLA_out.csv')
# cases -------------------------------------------------------------------
